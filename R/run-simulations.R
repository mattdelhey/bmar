...
library(parallel)
cl <- makeCluster(4)
clusterEvalQ(cl, {library(huge); library(xtable); source("simulations.R")})
...
sim.results <- simplify2array(parLapply(cl, 1:reps, function(i) {
    ...
    sim.sclfree <- huge.generator(n = 200, d = 300, graph = "scale-free")    
    sim.objects <- list(sim.random, sim.cluster, sim.band, sim.sclfree)
    sim.summarize(sim.objects, nlambda = 30, K = 5, stars.thresh = 0.05, stars.subsample.ratio = NULL, rep.num = 10)
}))
